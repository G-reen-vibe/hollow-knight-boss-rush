extends State
## Mid-air leap state. When the boss lands, transition to SlamLand.
...
func physics_process(_delta: float) -> void:
	...
	if _has_landed:
		return
	# Apply gravity (boss base already does this).
...
func _do_landed(b: Ravager) -> void:
	...
	b.velocity.y = 0.0
	b.sprite.rotation = 0.0
	# Spawn shockwave hitboxes on either side.
...
func _spawn_shockwave(b: Ravager, dir: int) -> void:
	...
	area.add_child(poly)
	# Damage any player in range, then disappear.
	...
	for hurt in area.get_overlapping_areas():
		if hurt is Hurtbox and hurt.is_player:
			var hit := Hitbox.new()
			hit.damage = 1
			hit.knockback_force = 360.0
			hit.hits_player = true
			hit.hits_enemies = false
			b.add_child(hit)
			hurt.take_hit(hit)
			hit.queue_free()
			break
...
func _do_screen_shake(_b: Node, _amount: float) -> void:
	# Simple: find the camera and shake it.
