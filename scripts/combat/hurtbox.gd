...
signal hit_taken(damage: int, source: Variant)
...
## Apply damage directly without needing a Hitbox node. Used for transient
## damage sources (shockwaves, projectiles) that don't have a persistent Hitbox.
func apply_damage(damage: int, knockback_force: float, source_position: Vector2) -> void:
        if _invincible:
                return
        hit_taken.emit(damage, null)
        var parent := get_parent()
        if parent is CharacterBody2D and knockback_force > 0.0:
                var dir: float = signf(parent.global_position.x - source_position.x)
                if dir == 0.0:
                        dir = -1.0 if parent.scale.x > 0.0 else 1.0
                var kb := Vector2(dir * knockback_force, -knockback_force * 0.25)
                if parent.has_method("apply_knockback"):
                        parent.apply_knockback(kb)
                else:
                        parent.velocity += kb
